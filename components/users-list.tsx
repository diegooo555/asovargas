    ...
    try {
      ...
      const processedClients =
        data?.map((client) => {
          ...
          const totalDebts = debts.reduce((sum: number, debt: any) => sum + Number.parseFloat(debt.total), 0)
          const pendingDebts = debts
            .filter((debt: any) => debt.status === "pending")
            .reduce((sum: number, debt: any) => sum + Number.parseFloat(debt.total), 0)
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Deudas Totales</TableHead>
            ...
            {filteredClients.length === 0 ? (
              ...
              filteredClients.map((client) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant={client.type_client === "associate" ? "default" : "secondary"}>
                      {client.type_client === "associate" ? "Asociado" : "Comprador"}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <span className="font-medium">${client.total_debts?.toFixed(2) || "0.00"}</span>
                  </TableCell>
                  <TableCell>
                    <span
                      className={`font-medium ${client.pending_debts && client.pending_debts > 0 ? "text-red-600" : "text-green-600"}`}
                    >
                      ${client.pending_debts?.toFixed(2) || "0.00"}
                    </span>
                  </TableCell>
